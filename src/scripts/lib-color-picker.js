...
const DOMProperties = (SuperClass) => class extends SuperClass {
  static get observedAttributes () {
    ...
    for (const i in DOMProps) observedAttributes.push((this.propertyAttributeNames || {})[DOMProps[i]] || DOMProps[i].toLowerCase())
  ...
  static __getPropertyNameByAttributeName (attrName) {
    ...
    for (const propName in attributeNames) if (attributeNames[propName] === attrName) return propName
    const DOMPropertyNames = this.constructor.DOMProperties || []
    for (const i in DOMPropertyNames) if (DOMPropertyNames[i].toLowerCase() === attrName) return DOMPropertyNames[i]
  }
...
const ReflectedProperties = (SuperClass) => class extends SuperClass {
  connectedCallback () {
    ...
    for (const i in this.constructor.reflectedProperties) {
...
const Properties = (SuperClass) => class extends ReflectedProperties(DOMProperties(ObservedProperties(SuperClass))) {
  ...
  static get propertyAttributeNames () {
    ...
    for (const propName in props) propValues[propName] = props[propName].attributeName || propName.toLowerCase()
    return propValues
  }
  ...
  static __getFilteredProperties (key, value) {
    const filteredProps = {}
    const props = this.properties
    for (const propName in props) if (props[propName][key] === value) filteredProps[propName] = props[propName]
  ...
  static __getPropertyValues (key) {
    const propValues = {}
    const props = this.properties
    for (const propName in props) propValues[propName] = props[propName][key]
    return propValues
  }
}
...
const TinyColor = /** @class */ (function () {
  ...
  TinyColor.prototype.toPercentageRgb = function () {
    const fmt = function (x) { return Math.round(bound01(x, 255) * 100) + '%' }
    return {
      r: fmt(this.r),
      g: fmt(this.g),
      b: fmt(this.b),
  ...
  TinyColor.prototype.toPercentageRgbString = function () {
    const rnd = function (x) { return Math.round(bound01(x, 255) * 100) }
    return this.a === 1
  ...
  TinyColor.prototype.toName = function () {
    ...
    for (let _i = 0, _a = Object.entries(names); _i < _a.length; _i++) {
      const _b = _a[_i]; const key = _b[0]; const value = _b[1]
      if (hex === value) {
        return key
      }
    }
...
class ColorPicker extends PropertiesChangedHandler(PropertiesChangedCallback(PropertyChangedHandler(Properties(HTMLElement)))) {
  ...
  set formats (val) {
    ...
    const formats = []
    for (const i in val) if (this.supportedFormats.indexOf(val[i]) !== -1) formats.push(val[i])
  ...
  _handleGridKeydown (e) {
    ...
    if (e.key.indexOf()) { if (e.key === 'ArrowLeft') return this.value = (this.selectedFormat === 'hsl') ? { ...hsl, ...{ s: hsl.s - 0.01 } } : { ...hsv, ...{ s: hsv.s - 0.01 } } }
